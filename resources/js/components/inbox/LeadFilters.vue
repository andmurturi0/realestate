...
<template>
    <div class="flex flex-wrap items-center gap-2 rounded-lg border bg-card p-3">
        ...
        <select v-model="filters.status" :class="inputClass" @change="emit('apply')">
            <option value="">Të gjitha statuset</option>
            <option v-for="(label, value) in statusOptions" :key="value" :value="value">{{ label }}</option>
        </select>

        <select v-if="agents" v-model="filters.assigned_to" :class="inputClass" @change="emit('apply')">
            <option value="">Të gjithë agjentët</option>
            <option v-for="agent in agents" :key="agent.id" :value="agent.id">{{ agent.name }}</option>
        </select>

        <input v-model="filters.date_from" type="date" :class="inputClass" @change="emit('apply')" />
        <span class="text-sm text-muted-foreground">–</span>
        <input v-model="filters.date_to" type="date" :class="inputClass" @change="emit('apply')" />
    </div>
</template>
