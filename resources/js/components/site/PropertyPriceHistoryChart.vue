...
const options = computed(() => ({
    ...
    xaxis: { type: 'datetime' },
    yaxis: { labels: { formatter: (value: number) => formatPriceCents(Math.round(value * 100)) } },
    tooltip: { y: { formatter: (value: number) => formatPriceCents(Math.round(value * 100)) } },
...
<template>
    <div v-if="history.length >= 2">
        <h2 class="mb-4 text-lg font-semibold">Historiku i çmimit</h2>
